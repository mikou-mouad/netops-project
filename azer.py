...
class YangValidationError(Exception):
    ...
    if "vlan-id" not in iface:
            ...
            f"[{site}] 'vlan-id' obligatoire manquant pour {iface.get('name')}")
    raise YangValidationError(
            f"[{site}] 'vlan-id' obligatoire manquant pour {iface.get('name')}")
                vlan_id = iface["vlan-id"]
    if not isinstance(vlan_id, int) or not (1 <= vlan_id <= 4094):
        raise YangValidationError(
            f"[{site}] vlan-id hors plage (1-4094) pour {iface.get('name')} : {vlan_id}")
    ...
    if enabled not in ALLOWED_STATES:
        raise YangValidationError(
            f"[{site}] 'enabled' doit etre 'up' ou 'down' pour {iface.get('name')} : {enabled}")
